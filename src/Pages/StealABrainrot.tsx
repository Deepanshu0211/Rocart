...
async function fetchProducts(category: string = "All", searchTerm: string = "") {
  const query = `
    {
      collection(id: "gid://shopify/Collection/647388070173") {
        id
        products(first: 10) {
          edges {
            node {
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className={`text-xl ${sectionId === 'hotItems' ? 'text-red-500' : sectionId === 'bestSellers' ? 'text-yellow-500' : sectionId === 'summerSpecials' ? 'text-blue-500' : sectionId === 'memes' ? 'text-purple-500' : sectionId === 'skins' ? 'text-green-500' : sectionId === 'bundles' ? 'text-pink-500' : 'text-orange-500'}`}>
          ...
          <div className="flex gap-4 overflow-x-auto scrollbar-hide pb-4">
            {visibleProducts.length > 0 ? (
              visibleProducts.map((product, idx) => (
                <div
                  key={`${sectionId}-${idx}`}
                  className="flex-shrink-0 w-[auto] bg-[#0a1612] bg-[url('/icon/itembg.png')] bg-cover bg-center bg-no-repeat rounded-2xl overflow-hidden transition-all cursor-pointer group"
                >
        ...
        {!loading && !error && products.length > 0 && (
          <>
            ...
            {(!activeSection || activeSection === "summerSpecials") && (
              ...
              {(!activeSection || activeSection === "brainrot") && (
                renderSection("brainrot", "Brainrot", "/icon/brainrot.png", products)
              )}
